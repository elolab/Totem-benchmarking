...
## This is the original code of dynverse_Slingshot
...
run_fun <- function(expression, parameters, priors, verbose, seed) {
    ...
    dimred <- dyndimred::dimred_landmark_mds(x = expression,ndim = 5)
    ...
    centroids <- do.call(rbind,lapply(split(1:nrow(rd),labels),function(x) apply(rd[x,,drop=FALSE],2,mean)))
    
    dm <- as.matrix(dist(centroids))
    
    cluster_network <- lineages %>%
        map_df(~ tibble(from = .[-length(.)], to = .[-1])) %>%
        unique() %>%
        mutate(
            length = dm[cbind(from, to)],
            directed = TRUE
        )
...
ti_slingshoti <- dynwrap::create_ti_method_r(
    definition = definition,
    run_fun = run_fun
)
...
models <- infer_trajectories(data,list(ti_slingshoti()
),verbose = TRUE)
